...
<c:choose>
   <c:when test="${action == 'status'}">
      <c:set var="queryParams" value="action=status&statusParam=${searchStatus}" />
   </c:when>
   <c:when test="${action == 'category'}">
      <c:set var="queryParams" value="action=category&categoryId=${searchCategory}" />
   </c:when>
   <c:otherwise>
      <c:set var="queryParams" value="action=${action}&keyword=${keyword}" />
   </c:otherwise>
</c:choose>
