...
<div class="container-fluid mt-4">
  ...
  <div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div class="d-flex">
      <form class="form-inline mr-2" id="searchForm" action="${pageContext.request.contextPath}/businessdocument/OutboundList.do" method="get">
        ...
        <!-- 상태 검색 단일 드롭다운 -->
        <select name="statusOption" id="statusSelect" class="form-control mr-2" style="display: none;">
          <option value="">상태 선택</option>
          <option value="1/대기" ${param.approveStep == '1' && param.approveStatus == '대기' ? 'selected' : ''}>1차 결재 대기</option>
          <option value="1/검토" ${param.approveStep == '1' && param.approveStatus == '검토' ? 'selected' : ''}>1차 결재 검토</option>
          <option value="1/승인" ${param.approveStep == '1' && param.approveStatus == '승인' ? 'selected' : ''}>1차 결재 승인</option>
          <option value="2/대기" ${param.approveStep == '2' && param.approveStatus == '대기' ? 'selected' : ''}>2차 결재 대기</option>
          <option value="2/검토" ${param.approveStep == '2' && param.approveStatus == '검토' ? 'selected' : ''}>2차 결재 검토</option>
          <option value="2/승인" ${param.approveStep == '2' && param.approveStatus == '승인' ? 'selected' : ''}>2차 결재 승인</option>
          <option value="/반려" ${param.approveStatus == '반려' ? 'selected' : ''}>반려</option>
        </select>

        <!-- hidden 값 -->
        <input type="hidden" name="approveStep" id="approveStepHidden" value="${param.approveStep}" />
        <input type="hidden" name="approveStatus" id="approveStatusHidden" value="${param.approveStatus}" />
        <input type="hidden" name="keyword" id="keywordHidden" />
        <input type="hidden" name="page" id="pageInput" value="${pageInfo.currentPage}" />
        ...
        <table class="table table-bordered text-center" width="100%" cellspacing="0">
          ...
          <c:forEach var="doc" items="${documentList}">
            <tr>
              ...
              <td><fmt:formatNumber value="${doc.outboundTotalAmount}" pattern="#,##0" />원</td>
              ...
              <td>
                <a href="${pageContext.request.contextPath}/businessdocument/outboundDetail.do?documentId=${doc.documentId}" class="btn btn-sm btn-outline-secondary">상세</a>
              </td>
            </tr>
          </c:forEach>
          </tbody>
      ...
      <nav aria-label="Page navigation">
        <ul class="pagination justify-content-center">
          <c:if test="${pageInfo.currentPage > 1}">
            <li class="page-item"><a class="page-link" href="javascript:void(0);" onclick="goToPage(1)">&laquo;</a></li>
            <li class="page-item"><a class="page-link" href="javascript:void(0);" onclick="goToPage(${pageInfo.currentPage - 1})">&lsaquo;</a></li>
          </c:if>
          ...
          <c:if test="${pageInfo.currentPage < pageInfo.totalPage}">
            <li class="page-item"><a class="page-link" href="javascript:void(0);" onclick="goToPage(${pageInfo.currentPage + 1})">&rsaquo;</a></li>
            <li class="page-item"><a class="page-link" href="javascript:void(0);" onclick="goToPage(${pageInfo.totalPage})">&raquo;</a></li>
          </c:if>
        </ul>
      </nav>
    </div>
