...
<input type="hidden" name="originalPwd" value="${ requestScope.employee.userPwd }">
...
<table id="outer" align="center" width="700" cellspacing="5" cellpadding="0">
	...
	<%-- input 태그의 name 속성의 이름은 employee.dto.employee 클래스의 property 명과 같게 함 --%>
		<td><input type="text" name="userId" id="userId" value="${ requestScope.employee.userId }" readonly>			
		</td></tr>
	<tr><th>사진첨부</th>
		<td>
		<c:if test="${ !empty requestScope.employee.photoFileName }">
			<div id="myphoto" style="margin:0; width:150px; height:160px; padding:0; border:1px solid navy;">
				<%-- 사진 첨부가 없을 경우를 위한 미리보기용 이미지 출력되게 함 --%>
				<img src="/first/resources/photoFiles/${ requestScope.employee.photoFileName }" id="photo" 
	...
	<tr><th>*이름</th>
		<td><input type="text" name="userName" id="userName" value="${ requestScope.employee.userName }" readonly></td></tr>
	<tr><th>*성별</th>
		<td>
			<c:if test="${ requestScope.employee.gender eq 'M' }">
				<input type="radio" name="gender" value="M" checked> 남자 &nbsp;
				<input type="radio" name="gender" value="F"> 여자 
			</c:if>
			<c:if test="${ requestScope.employee.gender eq 'F' }">
				<input type="radio" name="gender" value="M" > 남자 &nbsp;
				<input type="radio" name="gender" value="F" checked> 여자 
			</c:if>
		</td></tr>
	<tr><th>*나이</th>
		<td><input type="number" name="age" min="19" max="100" value="${ employee.age }" ></td></tr>
	<tr><th>*전화번호</th>
		<td><input type="tel" name="phone" value="${ employee.phone }"></td></tr>
	<tr><th>*이메일</th>
		<td><input type="email" name="email" value="${ employee.email }"></td></tr>
	<tr><th colspan="2">
		<input type="submit" value="수정하기"> &nbsp; 
		<input type="reset" value="수정취소"> &nbsp;
		
		<c:url var="mdel" value="mdelete.do">
			<c:param name="userId" value="${ requestScope.employee.userId }"></c:param>
		</c:url>
		<a href="${ mdel }">탈퇴하기</a>
		<%-- <a href="mdelete.do?userId=${ requestScope.employee.userId }">탈퇴하기</a> --%>
